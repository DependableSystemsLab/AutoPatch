...
The process is almost the same as before and only the two final steps (i.e., *Adding AutoPatch Workspace* and *Flash and Execute*) are different. To run the hotpatch generated by AutoPatch on the desktop computerو we provided a folder named [PCEvaluation](PCEvaluation), which contains **4 main files**:
