...
For generating the hotpatch (a functional equivalent patch that can be generated by one of the inserted trampolines, use the generated `.bc` file from prior step to run `analysis.sh` to get the `.o` file (see [Scripts](Scripts)), which represents the patched executable, capable on running on the targeted hardware.
