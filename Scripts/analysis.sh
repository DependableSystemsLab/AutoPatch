...
# HOTPATCH_DIRECTORY=~/AutoPatch/Testcases/CVE-Lists/Results/Hotpatch_CVE_10063.ll
...
HOTPATCH_DIRECTORY=~/AutoPatch/Testcases/CVE-Lists/Results/Hotpatch_CVE_${CVEid}.ll
...
"${OPT}" -S -enable-new-pm=0 -load "${LLVMAUTOPATCHSECOND}" -O1 -time-passes -patching -func-patch-name=prvCheckOptions -line-num-patch=18,19,20,21 -output-dir=$HOTPATCH_DIRECTORY -cve-id="${CVEid}" -type-offpatch=filter -type-patch=drop <"${BC_FILE}"> /dev/null
